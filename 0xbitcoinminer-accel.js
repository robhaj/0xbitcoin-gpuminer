...
module.exports = {
  ...
  {
    console.log('cuda ', this.useCUDA)
    if (this.useCUDA) {
      Miner = require('./build/Relase/gpuminer');
    }
  ...
  async collectMiningParameters(minerEthAddress, miningParameters, miningStyle) {
    ...
    try {
      ...
      await this.updateAddonParameters(miningParameters, miningStyle)
  ...
  async updateAddonParameters(miningParameters, miningStyle) {
    ...
    if (miningStyle == 'pool' && this.challengeNumber != null) {
      ...
      //   bResume = true;
      Miner.setChallengeNumber(this.challengeNumber);
      bResume = true;
    }
    ...
    if (this.miningTarget == null || !this.miningTarget.eq(miningParameters.miningTarget)) {
      ...
      Miner.setDifficultyTarget("0x" + this.miningTarget.toString(16));
    }
   ...
   async collectDataFromContract(contractData)
    {
        try {
            ...
            if (!contractData.challengeNumber || contractData.challengeNumber != challengeNumber) {
                ...
                Miner.setChallengeNumber(challengeNumber);
                bResume = true;
            }
            if (!contractData.miningTarget || contractData.miningTarget.cmp(miningTarget) != 0) {
                ...
                Miner.setDifficultyTarget("0x" + miningTarget.toString(16));
            }
  ...
  mineCoins(web3, miningParameters, minerEthAddress) {
    ...
    Miner.setMinerAddress(addressFrom);
